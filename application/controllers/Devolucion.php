...
class Devolucion extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Delocion';

		$this->load->model('model_devolucion');
	...
	*/
	...
	/*
	* Fetches the  devolucion data from the devolucion table 
	* this function is called from the datatable ajax function
	*/
	public function fetchDevolucionData()
	{
		$result = array('data' => array());

		$data = $this->model_devolucion->getDevolucionData();

		foreach ($data as $key => $value) {

			$count_total_item = $this->model_devolucion->countOrderItem($value['id']);
			...
			/*if(in_array('updateOrder', $this->permission)) {*/
				$buttons .= ' <a href="'.base_url('devolucion/update/'.$value['id']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			/*}*/
	...
	*/
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	$order_id = $this->model_devolucion->create();
        	
        	if($order_id) {
        		$this->session->set_flashdata('success', 'Creado Correctamente');
        		//redirect('devolucion/update/'.$order_id, 'refresh');
                        redirect('devolucion/', 'refresh');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('devolucion/create/', 'refresh');
        	}
        }
        else {
	...
	*/
	...
	/*
	* If the validation is not valid, then it redirects to the edit devolucion page 
...
*/
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	$update = $this->model_devolucion->update($id);
        	
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Agregado Correctamente');
        		//redirect('devolucion/update/'.$id, 'refresh');
                        redirect('devolucion/', 'refresh');
        	}
        	else {
        ...
        if ($dataLevel == "is_admin") {
           ...
           //este es para que se lleve el valor del select que esta seleciconado
        	...
        	$result = array();
        	$devolucion_data = $this->model_devolucion->getDevolucionData($id);

    		$result['order'] = $devolucion_data;
    		$devolucion_item = $this->model_devolucion->getDevolucionItemData($devolucion_data['id']);

    		foreach($devolucion_item as $k => $v) {
    			$result['order_item'][] = $v;
    		}
	...
	*/
	public function remove()
	{
        ...
        $response = array();
        if($order_id) {
            $delete = $this->model_devolucion->remove($order_id);
            if($delete == true) {
	...
	*/
	public function printDiv($id)
	{
		...
		if($id) {
			$order_data = $this->model_devolucion->getDevolucionData($id);
			$devolucion_items = $this->model_devolucion->getDevolucionItemData($id);
			    ...
			    <div class="row">
			      <div class="col-xs-12 table-responsive">
			        <table class="table table-striped">
			          ...
			          foreach ($devolucion_items as $k => $v) {
